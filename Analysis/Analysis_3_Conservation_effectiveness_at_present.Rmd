...
out_rds <- paste0(out_path,"result_3_conservation_effectiveness_existingPAs/Two_measures_per_PA_filter.rds")
...
if (!file.exists(out_rds)) {
  ...
  tmp_ID_PAs <- readRDS(path_PAs_sp) %>%
    select(ID_PAs) %>% 
    pull(.)
  
  rows_Nature_Remain_PAs <- readRDS(path_PAs_sp) %>%
    filter(category == "Natural Remains") %>%
    select(ID_PAs) %>%
    pull(.)

  rows_none_covered <- readRDS(path_PAs_sp) %>% 
    select(matches(splist)) %>% 
    rowSums(.) %>% 
    data.frame() %>% 
    setNames("richness") %>% 
    mutate(ID_PAs = tmp_ID_PAs) %>% 
    filter(richness==0) %>% 
    pull(.)
  
  rows_delPAs <- union(rows_Nature_Remain_PAs, rows_none_covered)
  
  df_PAs_sp <- readRDS(path_PAs_sp) %>%
    filter(!ID_PAs %in% rows_delPAs)
  ...
  df_PAs <- df_PAs_sp %>% 
    select(-matches(splist)) %>% 
    mutate(Area_km2 = No_pixel* (30/1000)^2)
  ...
  Measure_2 <- data.frame(Measure2_all_Endemic = Measure2_all_Endemic)
  col_sptype <- colnames(sp_info)[which(!grepl("gbif_name", colnames(sp_info)))]
  for (c_type in col_sptype) {
    tmp_df <- sp_info %>% 
      select(matches(paste0("gbif_name|", c_type)))
    ...
    for (i in 1:length(tmp_subtype)) {
      ...
      Measure_2 <- Measure_2 %>% 
        mutate(!!tmp_measure_name := tmp_Measure2)
    }
  }
...
out_RData <- paste0(out_path,"result_3_conservation_effectiveness_existingPAs/RData_two_measures_per_PA_filter.RData")
...
if (!file.exists(out_RData)) {
  ...
  path_PAs <- paste0(out_path,"result_3_conservation_effectiveness_existingPAs/Two_measures_per_PA_filter.rds")
  ...
  for (i in 1:length(measure_colnames)) {
    residual_colnames <- paste0("Residuals_", measure_colnames[i])
    v_measure <- Measure_1[,measure_colnames[i]]
    sel <- v_measure != 0
    fit <- lm(log(v_measure[sel]) ~ log(v_area[sel]))
    
    Measure_1 <- Measure_1 %>% 
      mutate(!!residual_colnames := NA)
    Measure_1[sel,ncol(Measure_1)] <- as.numeric(residuals(fit))
  }
  ...
  for (i in 1:length(measure_colnames)) {
    ...
    Measure_2 <- Measure_2 %>% 
      mutate(!!residual_colnames := NA)
    Measure_2[sel,ncol(Measure_2)] <- as.numeric(residuals(fit))
  }
...
PAs_effectiveness <- function(df, measure_colnames, group_colnames) {
  for (i in measure_colnames) {
    for (g in group_colnames) {
      ...
      measure_values <- df[,i]
      group_types <- df[,g]
      sel_NA <- !is.na(measure_values)
      measure_values <- measure_values[sel_NA]
      group_types <- group_types[sel_NA] 
      
      if (!grepl("Residuals_",i)) {
        sel_rich <- measure_values!=0
        measure_values <- measure_values[sel_rich]
        group_types <- group_types[sel_rich] 
      }
...
path_RData <- paste0(out_path,"result_3_conservation_effectiveness_existingPAs/RData_two_measures_per_PA_filter.RData")
load(path_RData)
...
group_colnames <- colnames(Measure_2)[which(grepl("^panda$|^type$", colnames(Measure_2)))]
